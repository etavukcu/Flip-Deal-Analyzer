...
function summarizeDeal(deal) {
  ...
  const categorizedCosts = otherCostsDetailed.reduce(
    (acc, item) => {
      ...
      if (label.includes('selling') || label.includes('commission')) acc.selling += amount;
      if (label.includes('financing') || label.includes('interest') || label.includes('points')) acc.financing += amount;
